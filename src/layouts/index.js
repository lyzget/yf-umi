...
export default class extends React.PureComponent {
  ...
  tabData = [
    ...
    {
      title: '转租转让',
      href: '/wx/sublease.html',
      icon: svgs[1],
      icon2: svgs2[1]
    },
    { title: '供求招聘', href: '/wx/info.html', icon: svgs[2], icon2: svgs2[2] }
  ];
  ...
  isActive(href, url = window.location.pathname) {
    if (window.location.pathname === '/' && href === '/wx/lease.html')
      return true;
  ...
  renderItem = (item, index) => {
    ...
    return (
      <TabBar.Item
        ...
        key={item.title}
        icon={item.icon}
        selectedIcon={item.icon2}
        selected={this.isActive(item.href)}
        onPress={() => {
          this.gotoTab(item.href);
        }}
      />
    );
  };
  ...
  render() {
    ...
    return (
      <div>
        ...
        {showTab && (
          <div id="main" className={styles.tabbar}>
            <TabBar
            ...
            >
              {this.tabData.map((item, index) => this.renderItem(item, index))}
            </TabBar>
          </div>
        )}
      </div>
